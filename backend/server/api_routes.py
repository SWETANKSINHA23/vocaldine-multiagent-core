...
app = FastAPI(title="VocalDine API", lifespan=lifespan)
...
async def create_sip_connection(request: SIPRequest):
    try:
        ...
        logger.debug(f"Connecting SIP trunk with numbers: {request.phone_numbers}")
        ...
        logger.debug(f"Created trunk: {trunk}")
        return {
            "status": "success",
...
async def create_sip_dispatch_rule(request: SIPDispatchRuleRequest):
    try:
        ...
        logger.debug(f"Creating dispatch rule for: {rule}")
        ...
        logger.debug(f"Dispatch rule created: {response}")
...
async def list_sip_dispatch_rules():
    try:
        ...
        logger.debug(f"Listed dispatch rules: {len(response.items)} items found")
        items = []
...
async def list_inbound_trunks():
    try:
        ...
        logger.debug(f"Listed inbound trunks: {len(response.items)} items found")
...
async def create_outbound_trunk(request: SIPOutboundTrunkRequest):
    try:
        ...
        logger.debug(f"Created outbound trunk: {response}")
...
async def delete_outbound_trunk(sip_trunk_id: str):
    try:
        ...
        response = await livekit_api.sip.delete_sip_trunk(request)
        logger.debug(f"Deleted outbound trunk: {response}")
        return {
            "status": "success",
...
async def create_sip_participant(request: SIPParticipantRequest):
    try:
        ...
        logger.debug(f"Created SIP participant: {response}")
